...
/// Service layer responsible for all Course-related **HTTP** calls — nothing else.
///
/// Responsibilities (and ONLY these):
///   - build requests against the JSONPlaceholder REST API
///   - parse JSON responses into [CourseModel]s
///   - translate transport/server failures into typed [AppException]s
///
/// It knows NOTHING about caching, connectivity policy, or UI state. Those
/// decisions live in [CourseRepository]. This keeps the service a thin,
/// replaceable adapter over the network.
///
...
/// expected/documented behavior. The repository + local cache give us a
/// realistic, persistent CRUD experience on top of it.
///
...
class CourseService {
  ...
  /// Wraps a request so that ALL transport-level failures surface as a
  /// [NetworkException] and all parsing/HTTP-status failures as an
  /// [ApiException]. The repository relies on this distinction.
  ...
  Future<CourseModel> addCourse({
    ...
    return _guard(() async {
      ...
      if (response.statusCode == 201) {
        ...
        // We build the result from the user's input so the UI shows exactly
        // what they typed.
        return CourseModel(
          id: data['id'] is int
              ? data['id'] as int
